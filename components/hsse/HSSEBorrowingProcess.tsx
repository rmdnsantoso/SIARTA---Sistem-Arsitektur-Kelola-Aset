          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            {modal.type === 'setujui' && (
              <>
                ...
                <div className="px-4 sm:px-6 pb-4 sm:pb-6 flex gap-3">
                  ...
                  <button onClick={handleConfirm} disabled={loading} className="flex-1 py-2 sm:py-3 bg-emerald-600 text-white rounded-xl font-bold text-sm hover:bg-emerald-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                    {loading && (
            ...
            {modal.type === 'tolak' && (
              <>
                ...
                <div className="px-4 sm:px-6 pb-4 sm:pb-6 flex gap-3">
                  <button onClick={() => setModal(null)} className="flex-1 py-2 sm:py-3 bg-white border border-gray-200 text-gray-700 rounded-xl font-bold text-sm hover:bg-gray-50 transition-colors">Batal</button>
                  <button onClick={handleConfirm} disabled={loading || !catatan.trim()} className="flex-1 py-2 sm:py-3 bg-red-600 text-white rounded-xl font-bold text-sm hover:bg-red-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                    {loading && (
                      <svg className="w-4 h-4 animate-spin text-white" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
