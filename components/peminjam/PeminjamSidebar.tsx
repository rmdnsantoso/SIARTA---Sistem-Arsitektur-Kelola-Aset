...
export default function PeminjamSidebar({ sidebarOpen, activeNav, setActiveNav, setSidebarOpen }: PeminjamSidebarProps) {
  const menus = [
  ...
  return (
    <>
        ...
        {sidebarOpen && (
          <div className="p-4 border-t border-gray-200 bg-gray-50">
            <div className="flex items-center gap-3">
              ...
              <div className="overflow-hidden">
                <p className="text-sm font-semibold text-gray-900 truncate">Ahmad</p>
                <p className="text-xs text-gray-500 truncate">Peminjam</p>
              </div>
            </div>
          </div>
        )}
      ...
      {/* Mobile Bottom Navbar (3 Tabs Grid) */}
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-gray-200 md:hidden grid grid-cols-3 px-2 py-1.5 shadow-lg">
        {menus.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xs text-center leading-tight mt-1 truncate w-full">
                {item.shortName}
              </span>
            </button>
          )
