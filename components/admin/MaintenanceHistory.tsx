...
interface HistoryTicket {
  ...
  status: 'Sedang Diperbaiki' | 'Selesai Diperbaiki' | 'Dimusnahkan'
  notes: string
  reporter: string
  photoUrl?: string
  photos?: string[]
    ...
    getMaintenanceHistory(currentPage, itemsPerPage, filterStatus, debouncedSearch).then(res => {
      if (res.success && res.data) {
        const adapted: HistoryTicket[] = res.data.map(r => ({
          ...
          status: r.status === 'Menunggu Tindakan' ? 'Sedang Diperbaiki' : r.status as 'Selesai Diperbaiki' | 'Dimusnahkan',
          notes: r.issue,
              ...
              <div className="flex justify-between items-start gap-3">
                ...
                <div className="shrink-0">
                  <span className={`inline-flex items-center px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md text-[10px] sm:text-[11px] uppercase tracking-wider font-bold border whitespace-nowrap ${
                    ...
                    item.status === 'Sedang Diperbaiki' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 pb-4 sm:pb-6 border-b border-gray-100">
                    <div>
                      ...
                      <span className={`inline-flex items-center px-3 py-1.5 rounded-md text-xs uppercase tracking-wider font-bold border ${
                        ...
                        selectedTicket.status === 'Sedang Diperbaiki' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                        'bg-gray-100 text-gray-600 border-gray-200'
                      }`}>
                      ...
                      <div className="-mt-1">
                        ...
                        {selectedTicket.reporter && (
                          <p className="text-[11px] text-gray-500 mt-1">
                            Pelapor: <span className="font-medium text-gray-700">{formatActorName(selectedTicket.reporter)}</span>
                          </p>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="flex gap-3 sm:gap-4">
                        ...
                        <div className="-mt-1">
                          ...
                          {selectedTicket.resolverName && (
                            <p className="text-xs text-gray-700 font-medium mt-1">Diselesaikan oleh: {formatActorName(selectedTicket.resolverName)}</p>
                          )}
                        </div>
                      </div>
                    )}
