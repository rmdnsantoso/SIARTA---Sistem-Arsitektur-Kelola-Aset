  ...
  return (
    <div className="space-y-6 font-sans relative">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-2 sm:p-4 bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[85vh]">
            ...
            <div className="px-3 sm:px-6 py-2.5 sm:py-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-2 sm:gap-3 shrink-0">
              ...
              <button onClick={handleSaveReport} className="px-4 py-2 text-sm font-bold text-white bg-red-600 rounded hover:bg-red-700 disabled:opacity-40 flex items-center justify-center gap-2"
                disabled={reportForm.items.length === 0 || (!(reportForm.items.length > 0 && reportForm.items[0].isSerialized) && !reportForm.notes) || isSubmitting}>
                {isSubmitting && (
                ...
                {isSubmitting ? 'Proses...' : 'Simpan & Eskalasi'}
              </button>
            </div>
          </div>
        </div>
      ...
      <LiveCameraCapture 
        ...
        photos={reportForm.photos} 
        onPhotosChange={(photos) => setReportForm(f => ({ ...f, photos }))} 
        maxPhotos={5} 
      />
